...
forms: [] = [];
  ...
  displayedColumns: string[] = ['nombreCliente', 'nombrePizza', 'cantidad', 'estadoPedido', 'fecha', 'actions'];
...
cargarforms(){
  this.apiService.ObtenerTodo('Pedido').subscribe(forms => {
  ...
  deleteforms(forms: any): void {
    Swal.fire({
      ...
      if (result.isConfirmed) {
       this.apiService.delete('Pedido', forms.id).subscribe(() => {
        this.cargarforms();
       })
      }
    });
  ...
  toggleIsActive(form : any){
    this.apiService.deleteLogic('Pedido', form.id).subscribe(() => {
      Swal.fire({
